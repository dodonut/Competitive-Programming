...
{
  if(val == 'H') return "Hearts";
  if(val == 'S') return "Spades";
  if(val == 'D') return "Diamonds";
  if(val == 'C') return "Clubs";
}

vector<int> doShuffles(int *s, int size, vector<vector<int>>& sh)
{
  vector<int> ans, now;
  int i,j, shuff, card;
  iter(i,1,53,1)
...
int main()
{
  ...
  while (testCases--) {
    ...
    i = 0;
    c = shf;
    while(c--) cin >> shufSeq[i++];

    now = doShuffles(shufSeq, shf, shuffles);
